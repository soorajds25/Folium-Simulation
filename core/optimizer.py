...
def apply_human_repeatability_filter(
    ...
    s_array: np.ndarray, 
    min_burst_dist: float = 3.0, 
    min_coast_dist: float = 10.0
) -> np.ndarray:
    ...
    for v_target in np.linspace(6.0, 14.0, 10):
        for delta_v in np.linspace(0.5, 4.0, 8):
            ...
            for i in range(n_nodes - 1):
                ...
                F_r = calculate_rolling_resistance(v, M_v, g, theta_array[i])
                ...
                if F_trac > 0:
                    ...
                    eta_m = get_motor_efficiency(torque, rpm)
                    P_elec = calculate_electrical_power(torque, rpm, is_propulsion=True)
                    I_batt, Vt = calculate_battery_state(P_elec, Voc, R_int)
                    
                    if I_batt > I_max:
                        P_elec = P_elec * (I_max / max(I_batt, 1e-6))
                        I_batt = I_max
            ...
            J = w_E * energy_j + w_T * (time_penalty**2)
